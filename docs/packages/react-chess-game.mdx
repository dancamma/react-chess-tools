...
function MyComponent() {
  const { game, history, makeMove, resetGame } = useChessGame();

  return (
    <div>
      <p>Turn: {game.turn()}</p>
      <button onClick={() => makeMove("e4")}>Play e4</button>
    </div>
  );
}
```
...
function MoveHistory() {
  const { history, methods } = useChessGameContext();
...
See [Theming/Presets](?path=/story/packages-react-chess-game-theming-presets) for available presets.
