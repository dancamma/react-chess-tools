...
<Meta title="Packages/react-chess-puzzle/Theming" />
...
All components accept standard HTML attributes (`className`, `style`, `id`, `data-*`, `aria-*`), making them compatible with any CSS approach including Tailwind CSS:

```tsx
<ChessPuzzle.Board className="rounded-lg shadow-lg" />
<ChessPuzzle.Reset className="px-4 py-2 bg-blue-500 text-white rounded">
  Try Again
</ChessPuzzle.Reset>
```
...
- Check out **[Game Theming](/docs/packages-react-chess-game-theming--docs)** for board and state color options
