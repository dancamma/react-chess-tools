...
function MoveCounter() {
  const { game, history } = useChessGameContext();
...
```tsx
<ChessStockfish.Root fen={fen} onFenChange={setFen}>
...
const {
  ...
  methods, // { makeMove, undoMove, resetGame, setPosition }
  isGameOver, // Boolean
} = useChessGameContext();
```
...
// Avoid - styling belongs on child components
<ChessGame.Root className="board" style={{ width: 400 }}>
```
