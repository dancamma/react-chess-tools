...
function MoveCounter() {
  ...
  return (
    <div>
      Moves: {history.length} · Turn: {game.turn() === "w" ? "White" : "Black"}
    </div>
  );
}
```
...
<ChessGame.Root timeControl={{ time: "5+3" }}>
  <ChessGame.Clock.PlayPause asChild>
    <button className="my-custom-button">Toggle Clock</button>
  </ChessGame.Clock.PlayPause>
  <ChessGame.Board />
</ChessGame.Root>
```

The button renders as your custom element but still triggers the play/pause action.
...
const {
  ...
  info: {
    ...
    normalizedEvaluation, // Score normalized to -1..+1 range
    bestLine, // Best principal variation line
    principalVariations, // All PV lines analyzed
    depth, // Search depth
    status, // "initializing" | "ready" | "analyzing" | "error"
    isEngineThinking, // Whether engine is currently analyzing
    hasResults, // Whether any results have been received
    error, // Error encountered, or null
  },
  methods: {
    startAnalysis, // Begin analysis
    stopAnalysis, // Stop analysis
    getBestMove, // Get best move from current analysis
...
const {
  ...
  status,             // "idle" | "delayed" | "running" | "paused" | "finished"
  ...
  currentPeriodIndex, // { white: number, black: number } - current period per player
  totalPeriods,       // Total number of periods
  currentPeriod,      // { white: Period, black: Period } - current period config
...
```tsx
// Good
<ChessGame.Root fen={fen} timeControl={{ time: "5+3" }}>
  <ChessGame.Board />
</ChessGame.Root>
...
```tsx
// Wrong - Sounds outside Root context
<ChessGame.Sounds />
<ChessGame.Root>
  <ChessGame.Board />
</ChessGame.Root>

// Correct - Sounds inside Root context
<ChessGame.Root>
  <ChessGame.Board />
  <ChessGame.Sounds />
</ChessGame.Root>
